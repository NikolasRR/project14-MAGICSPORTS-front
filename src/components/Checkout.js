...
function Checkout() {
  ...
  async function finishPurchase(ev) {
    ...
    try {
      const res = await axios.post(`${HTTP}checkout`, purchaseInfo, {
      ...
      <Container>
        ...
        <BuyerInfo onSubmit={(ev) => finishPurchase(ev)}>
          <Adress>
            ...
            <Apt
              placeholder="apt 602, casa 2..."
              onChange={(ev) => setApt(ev.target.value)}
              value={apt}
              id="apt"
              type="text"
            ></Apt>
          ...
          <Payment>
            ...
            <CCOwner
              ...
              onChange={(ev) => setCcOwner(ev.target.value)}
              value={ccOwner}
              id="ccOwner"
              type="text"
            ></CCOwner>
            ...
            <CCNumber
              ...
              onChange={(ev) => setCcNumber(ev.target.value)}
              value={ccNumber}
              id="ccNumber"
              type="text"
            ></CCNumber>
            ...
            <CCExpirationDate
              onChange={(ev) => setCcExpirationDate(ev.target.value)}
            ...
            <CCSecurityCode
              onChange={(ev) => setCcSecurityCode(ev.target.value)}
